...
def run_cnn_lstm():
    print("--- CNN-LSTM Hybrid Baseline ---")
    ...
    # 9 & 10. Build lightweight CNN-LSTM
    print("\nBuilding CNN-LSTM model...")
    input_shape = (X_train.shape[1], X_train.shape[2])
    
    model = Sequential()
    # 1D-CNN Block for local short-term feature extraction
    model.add(Conv1D(filters=32, kernel_size=3, activation='relu', input_shape=input_shape))
    model.add(MaxPooling1D(pool_size=2))
    
    # LSTM Block for temporal dependency tracking of the pooled features
    ...
    print(f"CNN-LSTM RMSE: {rmse:.2f} W")
    print(f"CNN-LSTM MAE:  {mae:.2f} W")
    
    # Output to text file
    with open('cnn_lstm_metrics.txt', 'w') as f:
        f.write("--- CNN-LSTM Final Results ---\n")
        f.write(f"R2: {r2:.4f}\nRMSE: {rmse:.2f}\nMAE: {mae:.2f}\n")
    ...
    plt.plot(real_preds[:window_sz], label='CNN-LSTM Predicted Power (t+1)', color='tab:red', linestyle='--', linewidth=2)
    plt.title('Single-Step Forecast Overlay: CNN-LSTM Actual vs Predicted', fontsize=14)
    plt.xlabel('Time Steps (10-minute intervals)')
    plt.ylabel('AC Power (W)')
    plt.grid(True, linestyle='--', alpha=0.6)
    plt.legend()
    plt.tight_layout()
    plt.savefig('cnn_lstm_pred.png', dpi=150)
    plt.close()
